...
LSTMState lstm(int num_hidden, Symbol indata,
  ...
  Symbol in_gate = Activation("", slice_gates[0], "sigmoid");
  Symbol in_transform = Activation("", slice_gates[1], "tanh");
  Symbol forget_gate = Activation("", slice_gates[2], "sigmoid");
  Symbol out_gate = Activation("", slice_gates[3], "sigmoid");
  Symbol next_c = (forget_gate * prev_state.c) + (in_gate * in_transform);
...
Symbol lstm_unroll(int num_lstm_layer, int seq_len, int input_size,
  ...
  for (int i = 0; i < num_lstm_layer; i++) {
    param_cells.push_back(
        LSTMParam(
            Symbol::Variable("l" + std::to_string(i) + "_i2h_weight"),
            Symbol::Variable("l" + std::to_string(i) + "_i2h_bias"),
            Symbol::Variable("l" + std::to_string(i) + "_h2h_weight"),
            Symbol::Variable("l" + std::to_string(i) + "_h2h_bias")));

    last_states.push_back(
        LSTMState(Symbol::Variable("l" + std::to_string(i) + "_init_c"),
                  Symbol::Variable("l" + std::to_string(i) + "_init_h")));
  }
  ...
  Symbol wordvec = Operator("SliceChannel")
      .SetInput("data", embed)
  ...
  for (int seqidx = 0; seqidx < seq_len; seqidx++) {
    ...
    for (int i = 0; i < num_lstm_layer; i++) {
      ...
      if (i == 0)
        ...
        dp_ratio = dropout;
      LSTMState next_state = lstm(num_hidden,
                                  hidden,
                                  last_states[i],
                                  param_cells[i],
                                  seqidx, i);
...
/*
   ...
   int num_hidden, int num_embed,
   int num_label, mx_float dropout) {
   int seqidx = 0;
   Symbol embed_weight = Symbol::Variable("embed_weight");
   Symbol cls_weight = Symbol::Variable("cls_weight");
   Symbol cls_bias = Symbol::Variable("cls_bias");
   ...
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_i2h_weight"));
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_i2h_bias"));
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_h2h_weight"));
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_h2h_bias"));
   param_cells.push_back(tmp);
   tmp.clear();
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_init_c"));
   tmp.push_back(Symbol::Variable("l" + std::to_string(i) + "_init_h"));
   last_states.push_back(tmp);
   tmp.clear();
   }
   Symbol data = Symbol::Variable("data");
   Symbol hidden = Embedding("embed", data, embed_weight,
   input_size, num_embed);
   for (int i = 0; i < num_lstm_layer; i++) {
   mx_float dp;
   if (i == 0)
