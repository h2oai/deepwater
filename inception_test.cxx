...
Symbol ConvFactory(Symbol data, int num_filter,
  ...
  Symbol conv_w("conv_" + name + suffix + "_w");
  Symbol conv_b("conv_" + name + suffix + "_b");
...
Symbol InceptionSymbol(int num_classes) {
  ...
  Symbol conv1 = ConvFactory(data, 64, Shape(7, 7), Shape(2, 2), 
                             Shape(3, 3), "conv1");
  Symbol pool1 = Pooling("pool1", conv1, Shape(3, 3), 
                         PoolingPoolType::max, false,
                         Shape(2, 2));

  Symbol conv2red = ConvFactory(pool1, 64, Shape(1, 1), Shape(1, 1), 
                                Shape(0, 0), "conv2red");
  Symbol conv2 = ConvFactory(conv2red, 192, Shape(3, 3), Shape(1, 1),
                             Shape(1, 1), "conv2");
  Symbol pool2 = Pooling("pool2", conv2, Shape(3, 3),
                         PoolingPoolType::max,
                         false, Shape(2, 2));

  Symbol in3a = InceptionFactoryA(pool2, 64, 64, 64, 64, 96,
                                  PoolingPoolType::avg, 32, "3a");
  Symbol in3b = InceptionFactoryA(in3a, 64, 64, 96, 64, 96,
                                  PoolingPoolType::avg, 64, "3b");
  Symbol in3c = InceptionFactoryB(in3b, 128, 160, 64, 96, "3c");

  Symbol in4a = InceptionFactoryA(in3c, 224, 64, 96, 96, 128,
                                  PoolingPoolType::avg, 128, "4a");
  Symbol in4b = InceptionFactoryA(in4a, 192, 96, 128, 96, 128, 
                                  PoolingPoolType::avg, 128, "4b");
  Symbol in4c = InceptionFactoryA(in4b, 160, 128, 160, 128, 160,
                                  PoolingPoolType::avg, 128, "4c");
  Symbol in4d = InceptionFactoryA(in4c, 96, 128, 192, 160, 192, 
                                  PoolingPoolType::avg, 128, "4d");
  Symbol in4e = InceptionFactoryB(in4d, 128, 192, 192, 256, "4e");

  Symbol in5a = InceptionFactoryA(in4e, 352, 192, 320, 160, 224,
                                  PoolingPoolType::avg, 128, "5a");
  Symbol in5b = InceptionFactoryA(in5a, 352, 192, 320, 192, 224,
                                  PoolingPoolType::max, 128, "5b");

  Symbol avg = Pooling("global_pool", in5b, Shape(7, 7),
                       PoolingPoolType::avg);
