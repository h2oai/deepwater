...
def export_train_graph(modelClass, optimizerClass,
    ...
    with graph.as_default():
        ...
        filename = "%s_%dx%dx%d_%d.pb" % (model.name, h, w, ch, class_n)
        ...
        meta = json.dumps({
            "inputs": {"batch_image_input": train.inputs.name,
                       "categorical_labels": train.labels.name},
            "outputs": {"categorical_logits": model.logits.name},
            "metrics": {"accuracy": accuracy.name,
        ...
        tf.train.export_meta_graph(saver_def=saver.as_saver_def())
...
if __name__ == "__main__":
    ...
    for (h, w) in zip(height, width):
        for ch in channels:
            for class_n in classes:
                modelClass = mlp.MultiLayerPerceptron
                m = generate_train_graph(modelClass,
                        optimizer.DefaultOptimizer, h, w, ch, class_n)
