...
public class Client {
  ...
  public XGRPCBackendModel createModel(
      XGRPCBackendSession session, String networkName, Map<String, Object> hashMap)
      throws Exception {
  ...
  public void deleteSession(XGRPCBackendSession session, BackendModel model) throws Exception {
    DeleteSessionRequest req =
        DeleteSessionRequest.newBuilder().setSession(asSession(session)).build();
  ...
  public void saveModel(XGRPCBackendSession session, XGRPCBackendModel model, String path)
      throws Exception {
    Map<String, Object> params = new HashMap<>();
    params.put("path", path);
    SaveModelRequest req =
        SaveModelRequest.newBuilder()
            .setSession(asSession(session))
  ...
  public void saveModelVariables(XGRPCBackendSession session, XGRPCBackendModel model, String path)
      throws Exception {
    Map<String, Object> params = new HashMap<>();
    params.put("path", path);
    SaveModelVariablesRequest req =
        SaveModelVariablesRequest.newBuilder()
            .setModel(asModel(model))
            .setSession(asSession(session))
            .putAllParams(asParams(params))
            .build();
  ...
  public void loadModelVariables(XGRPCBackendSession session, XGRPCBackendModel model, String path)
      throws Exception {
    Map<String, Object> params = new HashMap<>();
    params.put("path", path);

    LoadModelVariablesRequest req =
        LoadModelVariablesRequest.newBuilder()
            .setModel(asModel(model))
            .setSession(asSession(session))
            .putAllParams(asParams(params))
            .build();
  ...
  private Session.Builder asSession(XGRPCBackendSession session) {
    return ai.h2o.deepwater.backends.grpc.Session.newBuilder().setHandle(session.getHandle());
  }

  public void setParameters(
      XGRPCBackendSession session, XGRPCBackendModel model, Map<String, Object> params)
      throws Exception {

    SetModelParametersRequest req =
        SetModelParametersRequest.newBuilder()
            .setSession(asSession(session))
            .setModel(asModel(model))
            .putAllParams(asParams(params))
            .build();
  ...
  public Map<String, float[]> train(
      XGRPCBackendSession session,
      XGRPCBackendModel model,
    ...
    for (String key : m.keySet()) {
      Pair<float[], int[]> payload = m.get(key);
      Tensor.Builder tb =
              Tensor.newBuilder().setName(key)
                      .addAllFloatValue(Floats.asList(payload.getFirst()))
                      .setShape(toShape(payload.getSecond()))
                      .setType(DataType.DT_FLOAT32);
    ...
    for (String key : fetches) {
      Tensor.Builder tb = Tensor.newBuilder().setName(key);
  ...
  public Shape.Builder toShape(int[] shape) {
    Shape.Builder b = Shape.newBuilder();

    for (int i = 0; i < shape.length; i++) {
      b.addDim(Shape.Dim.newBuilder().setSize(shape[i]));
    }
    return b;
  }
  ...
  public XGRPCBackendSession createSession(RuntimeOptions runtimeOptions) throws Exception {
    // TODO ignore options
    CreateSessionRequest req = CreateSessionRequest.newBuilder().build();
